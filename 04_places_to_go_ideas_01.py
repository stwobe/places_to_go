# 04_places_to_go_ideas_01.py

'''
IDEAS:
	Add a functionality - choose a place and an activity -it will create a timeplan for you
	Add a functionality - choose distance you want to go - it will display different suggestions
	Add a functionality - choose mode of transport, or how much time you have
	This is a multimedia version of the book - like the one with Chris Barber doing the intro plus the old pic of Llancayo lighhouse             
	Include audio - pyscho-geography
	Include video - JD - quad
	Include video - BR - film
	Include photos - Me
	Add - Users/logins/passwords/favourites etc
	Search box - MongoDB style
	Visualization of final, front page - draw it on computer - 
'''
